...
def create_dev_set(df, dataset):
    if dataset == "zora":
        ...
        train_df, dev_df = train_test_split(
            reduced_df, test_size=0.1, stratify=reduced_df["sdg"], random_state=13
        )
        ...
        train_df.drop(
            [*sdg_1.head(1).index, *sdg_6.head(1).index, *sdg_11.head(1).index],
            inplace=True,
        )
        dev_df = pd.concat([dev_df, sdg_1.head(1), sdg_6.head(1), sdg_11.head(1)])
    else:
        train_df, dev_df = train_test_split(
            df, test_size=0.1, stratify=df["sdg"], random_state=13
        )
...
def main():
    ...
    train_path, dev_path, test_path = (
        f"{doc_path}/{dataset}_train.spacy",
