...
def plot_class_dists(subtitle, train_df, test_df, labels):
    ...
    if subtitle[0] == "Z":
        ...
        fontsize = 8
        tt_counts = {
            "Train (70%, 280 items)": train_counts,
            "Test (30%, 121 items)": test_counts,
        }
    else:
        ...
        if subtitle[0] == "O":
            tt_counts = {
                "Train (70%, 18'248 items)": train_counts,
                "Test (30%, 7'821 items)": test_counts,
            }
        else:
            tt_counts = {
                "Train (70%, 18'528 items)": train_counts,
                "Test (30%, 7'942 items)": test_counts,
            }
...
def main():
    ...
    zora_train_raw, zora_test_raw = train_test_split(
        zora_preproc, test_size=0.3, stratify=zora_preproc["sdg"], random_state=13
    )
    zora_train_raw.sort_index(inplace=True)
    zora_test_raw.sort_index(inplace=True)

    osdg_train_raw, osdg_test_raw = train_test_split(
        osdg_preproc, test_size=0.3, stratify=osdg_preproc["sdg"], random_state=13
    )
    ...
    concat_train_clean = pd.concat(
        [zora_train_clean, osdg_train_clean], ignore_index=False
    )
    concat_test_clean = pd.concat(
        [zora_test_clean, osdg_test_clean], ignore_index=False
    )
